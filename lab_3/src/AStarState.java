...
public class AStarState
{
	...
	HashMap<Location, Waypoint> closedWP = new HashMap<>();
	...
	public Waypoint getMinOpenWaypoint() {
		...
		for (Waypoint wp : openWP.values()) {
			if (wp.getTotalCost() < min) {
				waypoint = wp;
				min = wp.getTotalCost();
			}
		}
	 ...
	 * already in) the "open waypoints" collection. If there is not already an
	 * open waypoint at the new waypoint's location then the new waypoint is
	 * simply added to the collection. However, if there is already a waypoint
	 * at the new waypoint's location, the new waypoint replaces the old one
	 * <em>only if</em> the new waypoint's "previous cost" value is less than
	 * the current waypoint's "previous cost" value.
	 **/
	public boolean addOpenWaypoint(Waypoint newWP) {
		if (openWP.containsValue(newWP)) {
			if (openWP.get(newWP.getLocation()).getPreviousCost() >
		...
		return openWP.containsValue(newWP);
	}
	...
	public void closeWaypoint(Location loc) {
		...
		openWP.remove(loc, openWP.get(loc));
	}
